...
// 3. Select which bands you want to export (TCI_R, TCI_G, TCI_B RGB selected by default)
...
var bandDefinitions = {
  ...
  'True Color (TCI)': [
    ...
    {name: 'TCI_R', description: 'True Color Red (8-bit)', resolution: '10m'},

    {name: 'TCI_G', description: 'True Color Green (8-bit)', resolution: '10m'},

    {name: 'TCI_B', description: 'True Color Blue (8-bit)', resolution: '10m'}
...
function maskS2clouds(image) {
  ...
  // Bands that are scaled by 10000 (reflectance + some L2 products)
  ...
  var scaled = masked.select(scaleBands).divide(10000);

 

  // Keep other bands (TCI_R, TCI_G, TCI_B, SCL, QA bands, etc.) as-is

  var otherBands = masked.select(masked.bandNames().removeAll(scaleBands));

 

  return scaled.addBands(otherBands);
...
function runAnalysis(earliest, latest, statusLabel, bandCheckboxes) {
  ...
  // Decide visualization scaling:

  // If all selected bands are TCI_R/G/B, use 8-bit 0–255 range.

  var allTCI = selectedBands.length > 0 && selectedBands.every(function(b) {

    return (b === 'TCI_R' || b === 'TCI_G' || b === 'TCI_B');

  });

 

  var visParams = allTCI
  ...
  if (selectedBands.length === 1) {
    ...
    Map.addLayer(selectedImage, visParams, selectedBands[0] + ' (grayscale)');

  } else if (selectedBands.length === 2) {

    // Two bands - show first band as grayscale

    Map.addLayer(selectedImage.select([selectedBands[0]]), visParams,

                 selectedBands[0] + ' (grayscale)');
    ...
    Map.addLayer(selectedImage, rgbVis, visName + ' (RGB)');
...
for (var category in bandDefinitions) {
  ...
  for (var i = 0; i < bands.length; i++) {
    ...
    // Default to TCI bands for true color composite
